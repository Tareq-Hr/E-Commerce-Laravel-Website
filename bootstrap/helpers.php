...
function getUrl(){
	...
	$full_url = $http.$_SERVER['HTTP_HOST'].str_replace(array('\\','/index.php','/index.html'),'',$_SERVER['PHP_SELF']);
...
function getCategories(){
	...
	$categories = App\Categorie::where('parent',null)->get();
...
function list_actions($object){
	...
	if( method_exists($object, 'list_actions') ){
	 ...
	 if( Route::currentRouteName() == 'commande')
	 	$html .= '<a href="'.route('afficher_commande',$object->id).'" class="icon pl-3"><i class="fa fa-eye"></i></a>';
		...
		$html .= '<div class="card-header">';
			...
			$html .= '<div class="card-options">';
				if( isGranted($model) && Route::has(strtolower($model)."_create") )
                	$html .= '<a class="btn btn-link" href="'.route(strtolower($model)."_create").'"><i class="fa fa-plus"></i>&nbsp;Ajouter</a>';
			$html .= '</div>';
		$html .= '</div>';
